...
function process_files(yaml_dir, lammpstrj_dir)
    ...
    for yaml_file in yaml_files
        ...
        for t in 1:ts
            if t % 10 == 0
                plt = plot!(Yg_values[1:t],y_std_deviations[1:t],time_values[1:t], label="",st=scatter,mc=:red,xlims=(0.2,0.8),ylims=(0.1,0.4), title="(StD of y)/Ly vs. Yg/Ly over Time",xlabel="Yg/Ly",ylabel="std y/Ly",zlabel="time")
                frame(anim,plt) 
            end
        end
        gif(anim,replace(filename, r".yaml$" => ".gif"),fps=20)
    end
end
